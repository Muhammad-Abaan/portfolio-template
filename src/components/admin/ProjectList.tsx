...
export const ProjectList = ({ onEdit }: ProjectListProps) => {
  ...
  const { data: projects = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Try ordering by order_index, fallback to id if column missing
  ...
  const reorderMutation = useMutation({
    ...
    onError: (error) => {
      toast.error('Failed to reorder projects. Have you run the SQL migration to add order_index? ' + error.message);
    }
  });
