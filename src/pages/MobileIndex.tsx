...
const InsightsAccordion = ({ insights }: { insights: any[] }) => {
  const [activeIdx, setActiveIdx] = useState(0);

  if (!insights.length) {
    return <div className="py-12 text-center text-muted-foreground">No featured insights selected yet.</div>;
  }

  return (
    <div className="flex flex-row w-full h-[350px] gap-2">
      {insights.map((insight: any, idx: number) => {
          ...
          <>
            {insight.media_url ? (
              insight.media_type === 'video' ? (
                ...
                <img src={insight.media_url} alt={displayTitle} className="absolute inset-0 w-full h-full object-cover" />
              )
            ) : (
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 h-full flex flex-col justify-end">
              <motion.div
                initial={false}
              ...
              >
                <span className="text-[10px] font-medium tracking-widest text-primary uppercase whitespace-nowrap">
                  {displayType} • {displayDate}
                </span>
                {isCustom && (linkUrl.startsWith('http') || linkUrl.startsWith('www')) ? (
                  <a href={linkUrl} target="_blank" rel="noopener noreferrer">
                    <h3 className="text-sm font-bold text-white line-clamp-3">{displayTitle}</h3>
                  </a>
                ) : (
                  <Link to={linkUrl}>
                    <h3 className="text-sm font-bold text-white line-clamp-3">{displayTitle}</h3>
                ...
                {isCustom && (linkUrl.startsWith('http') || linkUrl.startsWith('www')) ? (
                  <a href={linkUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-neutral-400 mt-1 flex items-center gap-1">
                    Read article <ChevronRight className="w-3 h-3" />
                  </a>
                ) : (
                  <Link to={linkUrl} className="text-xs text-neutral-400 mt-1 flex items-center gap-1">
                    Read {displayType.toLowerCase()} <ChevronRight className="w-3 h-3" />
                  </Link>
                )}
              </motion.div>
            </div>
        ...
        return (
          <motion.div 
            ...
            className="relative rounded-2xl overflow-hidden border border-white/10 shadow-xl cursor-pointer"
          >
            {InnerContent}
          </motion.div>
        );
  ...
  return (
    <div style={{ perspective: "1000px" }} className="w-full flex justify-center py-4">
      <motion.div
        ...
        className="relative w-full max-w-4xl rounded-3xl bg-gradient-to-br from-[#1c1c1c] to-[#0a0a0a] border border-white/10 p-5 md:p-14 shadow-[0_30px_60px_rgba(0,0,0,0.8)]"
      >
        <div style={{ transform: "translateZ(30px)" }} className="flex flex-col gap-4 md:gap-10">
          {/* Top Section: Name & Intro */}
          <div className="text-center space-y-2 md:space-y-6">
            <h3 className="text-2xl md:text-5xl font-black bg-clip-text text-transparent bg-gradient-to-r from-white to-neutral-400 tracking-tight leading-tight">
              Your Name
            </h3>
            <p className="text-xs md:text-xl text-neutral-400 leading-relaxed max-w-3xl mx-auto font-light">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-12 text-left">
            {/* Left Column: Basic Info */}
            <div className="space-y-3 md:space-y-8">
              <div>
                <div className="text-[10px] md:text-xs font-bold text-primary tracking-widest uppercase mb-0.5">Location / Timezone</div>
                <div className="text-neutral-200 text-sm md:text-xl font-medium">Your City, Country</div>
              </div>
              <div>
                <div className="text-[10px] md:text-xs font-bold text-primary tracking-widest uppercase mb-0.5">Comm Link</div>
                <a href="mailto:your.email@example.com" className="text-neutral-200 text-sm md:text-xl font-medium hover:text-white transition-colors relative z-10 inline-block truncate w-full">
            ...
            <div className="space-y-3 md:space-y-6">
              <div className="text-[10px] md:text-xs font-bold text-primary tracking-widest uppercase mb-0.5 md:mb-1">Education Log</div>
              <div className="space-y-3 md:space-y-6">
                <div className="group">
                  <div className="text-sm md:text-lg font-semibold text-white group-hover:text-primary transition-colors">Your Degree [e.g. Computer Science]</div>
                  <div className="text-[10px] md:text-sm text-neutral-400 flex justify-between mt-0.5">
                    <span className="truncate pr-2">Your University Name</span>
                    <span className="text-neutral-500 font-medium shrink-0">Graduation Year</span>
                  </div>
                </div>
                <div className="group">
                  <div className="text-sm md:text-lg font-semibold text-white group-hover:text-primary transition-colors">Previous Education</div>
                  <div className="text-[10px] md:text-sm text-neutral-400 flex justify-between mt-0.5">
                    <span className="truncate pr-2">Your High School or College</span>
                    <span className="text-neutral-500 font-medium shrink-0">Grade | Year</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <main className="bg-background text-foreground relative">
      ...
      <motion.section 
        ...
        className="sticky top-0 h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-background z-0"
      >
        ...
        <motion.div 
          className="absolute bottom-12 inset-x-0 flex flex-col items-center justify-center gap-2 text-muted-foreground"
          animate={{ y: [0, 10, 0] }}
      ...
      {/* CARD 2: PHILOSOPHY */}
      <section className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden px-4 bg-[#0a0a0a] rounded-t-[2rem] md:rounded-t-[3rem] border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.7)] z-10">
      ...
      {/* CARD 3: TECH STACK */}
      <section className="sticky top-0 h-screen w-full bg-[#111111] rounded-t-[2rem] md:rounded-t-[3rem] border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.7)] flex flex-col items-center justify-center overflow-hidden z-20">
      ...
      <section className="sticky top-0 h-screen w-full bg-[#0c0c0c] rounded-t-[2rem] md:rounded-t-[3rem] border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.7)] flex flex-col justify-center px-4 md:px-8 z-30">
        <div className="max-w-6xl mx-auto w-full">
          ...
          <div className="w-full">
            <InsightsAccordion insights={featuredInsights} />
          </div>
        </div>
      </section>
      ...
      <section className="sticky top-0 h-screen w-full bg-[#151515] rounded-t-[2rem] md:rounded-t-[3rem] border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.7)] flex flex-col justify-center px-4 md:px-8 z-40">
        <div className="max-w-6xl mx-auto w-full">
          <TradingCard />
        </div>
      </section>
      ...
      <section className="sticky top-0 h-screen w-full bg-background rounded-t-[2rem] md:rounded-t-[3rem] shadow-[0_-20px_50px_rgba(0,0,0,0.8)] border-t border-white/10 flex flex-col justify-between overflow-hidden z-50">
