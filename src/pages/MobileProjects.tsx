...
const MobileProjects = () => {
  ...
  return (
    <main className="container pt-28 pb-12 md:pt-32 md:pb-24">
      ...
      >
        {isLoading ? (
          ...
          projects.map((p) => {
            ...
            >
              <Card className="h-full flex flex-col group bg-gradient-to-br from-white/[0.08] to-transparent backdrop-blur-2xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden hover:border-white/20 hover:bg-white/[0.05] transition-all duration-500 rounded-3xl relative">
                {p.title === projects[0]?.title && (
                  <span 
                    onClick={(e) => { e.preventDefault(); (window as any).findRickRollNumber?.(4); }}
                    className="absolute top-4 right-4 text-[10px] opacity-20 cursor-pointer select-none z-50 mix-blend-difference"
                  >
                    4
                  </span>
                )}
